...
const detenusValidation = (body) => {
    const detenusSchema = Joi.object({
            ...
            image: Joi.string().min(3).max(100).trim().required().messages({
                'string.base': 'L\'image doit être une chaîne de caractères',
                'string.empty': 'L\'image ne doit pas être vide',
                'string.min': 'L\'image doit contenir au moins {#limit} caractères',
                'string.max': 'L\'image doit contenir au plus {#limit} caractères',
...
export default detenusValidation